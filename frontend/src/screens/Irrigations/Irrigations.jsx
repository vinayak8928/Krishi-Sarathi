...
const Irrigations_LendScreen = () => {
    ...
    const irrigationsList = productLendMachines.filter(machine => machine.category === 'Irrigations');
    const showMore = () => {
        if (numberOfItems + 3 <= irrigationsList.length) {
            setNumberOfItems(numberOfItems + 3)
        } else {
            setNumberOfItems(irrigationsList.length)
        }
    }

    // Filter machines whose names contain the word "Threshers"
    // const irrigationsList = productLendMachines.filter(machine => machine.category.toLowerCase().includes('irrigations'));
                        ...
                        : error ? <Message variant='danger'>{error}</Message>
                            : (
                                <Row>
                                    ...
                                    {numberOfItems >= irrigationsList.length ? (
                                    <Alert
